...
def main():
    tdoas = []
    files = get_files(root='../data/data7.16/pb/14')
    print(files)
    print('Loading files')
    for i in range(len(files)):
        ...
        for j in range(len(slices)):
            with open(slices[j], 'r') as f:
                ...
                datas.append(data)
            print(slices[j])
        print(f'shape of data:{np.array(datas).shape}')
        print('Calling matlab')
        print(os.path.dirname(slices[0]))
        tdoa_ = np.array(tdoa(trigger_time, datas)).squeeze()
        print('Finished')
